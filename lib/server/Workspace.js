...
function updateModel(_this) {
   ...
   var upd = {
      ...
      shape: _this.shape,
      html: _this.html,
      clients: _.map(_this.clients, extractID),
      inner: _.map(_this.inner, extractID)
   };

   model.fetch(_this.racer.path, function() {
//      model.setDiffDeep(_this.racer.path + '.workspaces.' + _this.id, upd);
      model.setDiff(_this.racer.path + '.workspaces.' + _this.id + '.id', upd.id);
      model.setDiffDeep(_this.racer.path + '.workspaces.' + _this.id + '.shape', upd.shape);
      model.setDiffDeep(_this.racer.path + '.workspaces.' + _this.id + '.html', upd.html);
      model.setArrayDiff(_this.racer.path + '.workspaces.' + _this.id + '.clients', upd.clients);
      model.setArrayDiff(_this.racer.path + '.workspaces.' + _this.id + '.inner', upd.inner);
   });
}
